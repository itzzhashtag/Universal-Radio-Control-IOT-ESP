...
void DataWork() 
{
  ...
  Serial.printf("L(%3d,%3d) R(%3d,%3d) | LB:%d RB:%d | LS:%d RS:%d | SW1:%d SW2:%d | P1:%d | BAT:%d%% | Status:%s\n",
  data.Lx, data.Ly, data.Rx, data.Ry,data.LBt, data.RBt, data.LABt, data.RABt,data.TSW1, data.TSW2,data.Pot1,BaT,(Stat ? "true" : "false")); 
...
void setup() 
{
  ...
  lcd.setCursor(0, 2); lcd.print("    ESP-NOW v0.6    ");
  lcd.setCursor(0, 3); lcd.print("====================");
