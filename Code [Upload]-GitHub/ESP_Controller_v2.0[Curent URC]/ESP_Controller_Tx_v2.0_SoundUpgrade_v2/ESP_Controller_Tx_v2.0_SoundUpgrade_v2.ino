...
//  NOTE: no noTone() at end — tone() self-expires; hard cut = click
void playSweep(int startFreq, int endFreq, int durationMs, int steps = 30) {
  ...
  // No noTone() here — last tone() duration expires cleanly on its own
}

// ── Single tone with optional gap after ──────────────────────
//  tone() with a duration argument self-terminates; no noTone() needed
void playTone(int freq, int durationMs, int gapMs = 0) {
  tone(PIN_BUZZ, freq, durationMs);
  delay(durationMs + gapMs);
  // No noTone() here — duration handles cutoff without a click
}

// ── Soft stop: tiny rapid descending sweep to ~inaudible freq ─
//  Prevents the abrupt square-wave cut that causes the "click" pop.
//  Call this at the very end of any sound profile instead of noTone().
void softStop() {
  // Sweep from a low frequency down to ~80 Hz over 18 ms
  // By the time we reach 80 Hz the amplitude is perceptually gone
  for (int f = 300; f >= 80; f -= 30) {
    tone(PIN_BUZZ, f, 3);
    delay(3);
  }
  noTone(PIN_BUZZ);  // safe to cut at 80 Hz — click is inaudible here
}
...
void toneConnected() {
  ...
  delay(25);                        // snap silence = punch (no noTone click)
  playTone(2200, 70,  35);          // bip
  playTone(2600, 110, 0);           // BIP (longer = confident)
  softStop();                       // gentle tail-off instead of hard cut
}
...
void toneBoot() {
  ...
  delay(35);                        // breath before final note
...
void soundJoyClick() {
  ...
  // No softStop here — sweep already ends at high freq which decays
  // fast enough on its own; adding softStop would feel sluggish on rapid clicks
}
...
void soundShoulderBtn() {
  ...
  delay(10);                        // gap between thunk and pip
  playSweep(1200, 1800, 40, 10);    // rising pip
  // No softStop — pip ends naturally at 1800 Hz, decays quickly on speaker
}

// ── Toggle switch ON — ascending blip ────────────────────────
void soundToggleOn() {
  playSweep(800, 1600, 55, 12);
  // No softStop — ends high, speaker cone damps naturally
}

// ── Toggle switch OFF — descending blip ──────────────────────
void soundToggleOff() {
  playSweep(1600, 800, 55, 12);
  softStop();  // ends low — without this the low-freq cut IS audible
}
...
void soundPotTick(int rawValue) {
  ...
  // No softStop — very short sweep, decays on its own fast enough
}
// ================================================================
//  ESP-NOW CALLBACKS
// ================================================================
