...
class ESPNowReceiver
{
  ...
  // ─────────────────────────────────────────────────────────────
  static void _handlePacket(const uint8_t *mac,
                             const uint8_t *buf, int len)
  {
    ...
    Serial.printf("[RX←PKT] L(%+3d,%+3d) R(%+3d,%+3d) | LA:%d RA:%d LB:%d RB:%d | S1:%d S2:%d | P1:%3d P2:%3d | BAT:%3d%% | Link:%s\n",
      ...
      _instance->data.TSW1, _instance->data.TSW2, _instance->data.Pot1, _instance->data.Pot2,
      _instance->data.BAT, _instance->connected_flag ? "Connected" : "Dead"); // show status
  }
...
void setup()
{
  ...
  // receiver.begin("D4:E9:F4:A4:C9:7C");
