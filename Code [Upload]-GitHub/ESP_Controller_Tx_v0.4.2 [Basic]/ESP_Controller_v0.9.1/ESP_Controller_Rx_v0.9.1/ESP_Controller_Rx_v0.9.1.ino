...
// ================================================================
#define LINK_TIMEOUT_MS  300   // ms.  300ms = ~15 missed packets at 50Hz.
...
void applyDrive(int ly, int rx) {
  ...
  driveMotor(true,  leftPWM);   // LEFT  motor
...
// ================================================================
void onRABt() {
  Serial.println("[ACTION] RABt triggered — add your feature here");
}
...
void onRBt(bool pressed) {
  if (pressed) Serial.println("[ACTION] RBt pressed — add your feature here");
  else         Serial.println("[ACTION] RBt released");
}
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  ...
  // Send ACK back to TX so it can light up the green LED
  esp_now_send(info->src_addr, (uint8_t *)&ackPacket, sizeof(ackPacket));
}
...
void processInputs() {
  ...
  // ── RABt stub — edge detect (press only) ─────────────────
  if (rxData.RABt && !prevRABt) onRABt();
...
void linkWatchdog() {
  ...
  if (linked != wasLinked) {
    if (!linked) {
      Serial.println("[LINK] !! SIGNAL LOST — stopping motors and disarming !!");
      memset(&rxData, 0, sizeof(rxData));  // zero all controls
      stopMotors();
  ...
  // Periodic heartbeat print when active
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("\n==============================");
  Serial.println("  URC RX v0.9.1 — Robot Boot");
  Serial.println("==============================");
