...
Adafruit_ADS1115 ads;   // Battery voltage divider (A0, input-only)
...
#define DISP_SERIAL  Serial    // change to Serial2 if needed
// ================================================================
//  PIN DEFINITIONS
...
int readBatPct(float &vOut)
{
  ...
  vOut = vADC * (147.0f / 47.0f) * BAT_CAL_FACTOR; 
...
void initDisplaySerial()
{
  // Serial (Serial0) is already begun in setup() for debug.
  // If using Serial2, uncomment:
  // Serial2.begin(38400, SERIAL_8N1, 16, 17);  // RX=16 TX=17 (adjust pins)
  // But GPIO16/17 are TSW2/TSW1 in your layout!
  // Safer: use Serial2 on GPIO 25/26 — but 26 = PIN_LABt ...
  // Simplest: use Serial (USB) and monitor via Serial2 or just drop USB debug.
  // Default here: Serial0 at 38400. Adjust baud to match Nano below.
}
...
void sendDisplayPacket()
{
  ...
  // Joystick axes: offset by 99 so range is 0-198 (no negatives over wire)
  pkt[1]  = (uint8_t)(txData.Lx + 99);
  pkt[2]  = (uint8_t)(txData.Ly + 99);
  pkt[3]  = (uint8_t)(txData.Rx + 99);
  pkt[4]  = (uint8_t)(txData.Ry + 99);

  pkt[5]  = (uint8_t)txData.Pot1;
  pkt[6]  = (uint8_t)txData.Pot2;
  pkt[7]  = (uint8_t)txData.BAT;

  // Pack all booleans into one byte
  pkt[8]  = (txData.LABt    ? 0x01 : 0)
           |(txData.RABt    ? 0x02 : 0)
           |(txData.LBt     ? 0x04 : 0)
           |(txData.RBt     ? 0x08 : 0)
           |(txData.TSW1    ? 0x10 : 0)
  ...
  // Reserved bytes
  for (uint8_t i = 9; i < 15; i++) pkt[i] = 0x00;

  // XOR checksum over bytes 1..14
  uint8_t cs = 0;
  for (uint8_t i = 1; i < 15; i++) cs ^= pkt[i];
  pkt[15] = cs;

  DISP_SERIAL.write(pkt, 16);
}
// ================================================================
//  SETUP
// ================================================================
...
void loop()
{
  ...
  //batteryManager(); 
  sendDisplayPacket();   // ← NEW: push data to Nano display
