...
public:
  ...
  static void _handlePacket(const uint8_t *mac, const uint8_t *buf, int len)
  {
    ...
    Serial.printf(" L(%+3d,%+3d) R(%+3d,%+3d) | LA:%d RA:%d LB:%d RB:%d | S1:%d S2:%d | P1:%3d | Link : %s\n",
      ...
      _instance->data.TSW1, _instance->data.TSW2,_instance->data.Pot1, _instance->connected_flag ? "Connected" : "Dead"); // show status
  }
