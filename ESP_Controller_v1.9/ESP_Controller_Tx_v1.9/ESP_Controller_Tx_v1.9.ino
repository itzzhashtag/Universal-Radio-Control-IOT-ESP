...
//    1× Passive Buzzer    GPIO 19   (100Ω → GND)
//
//  ── UART to Nano ─────────────────────────────────────────────────
...
// ── Outputs ──────────────────────────────────────────────────────
#define PIN_LED_R 13  // Red  LED  HIGH = ON
#define PIN_LED_G 12  // Green LED HIGH = ON
...
void checkBeeps() {
  ...
  // Pot pitch feedback — only fires when pot actually moves
  // Maps 0..100 → 400..2400 Hz
  if (pPot1 != -1 && abs(txData.Pot1 - pPot1) > 2) {
    int freq = map(txData.Pot1, 0, 100, 400, 2400);
    tone(PIN_BUZZ, freq, BEEP_DUR);
  }
  if (pPot2 != -1 && abs(txData.Pot2 - pPot2) > 2) {
    int freq = map(txData.Pot2, 0, 100, 400, 2400);
    tone(PIN_BUZZ, freq, BEEP_DUR);
  }
