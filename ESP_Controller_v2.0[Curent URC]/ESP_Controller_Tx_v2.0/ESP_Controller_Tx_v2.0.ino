...
void checkBeeps() {
  ...
  static int pPot1Raw = -1, pPot2Raw = -1;  // raw ADC values now

  if (txData.LABt && !pLABt) beep();
  if (txData.RABt && !pRABt) beep();
  if (txData.LBt  && !pLBt)  beep();
  if (txData.RBt  && !pRBt)  beep();
  if (txData.TSW1 != pTSW1)  beep();
  if (txData.TSW2 != pTSW2)  beep();

  // Read raw ADC directly
  int rawP1 = analogRead(PIN_POT1);
  int rawP2 = analogRead(PIN_POT2);

  // Threshold of ~80 on 0-4095 scale ≈ ~2% movement — ignores noise
  if (pPot1Raw != -1 && abs(rawP1 - pPot1Raw) > 80) {
    int freq = map(rawP1, 0, 4095, 400, 2400);
    tone(PIN_BUZZ, freq, BEEP_DUR);
    pPot1Raw = rawP1;  // only update when threshold crossed
  }
  if (pPot2Raw != -1 && abs(rawP2 - pPot2Raw) > 80) {
    int freq = map(rawP2, 0, 4095, 400, 2400);
    tone(PIN_BUZZ, freq, BEEP_DUR);
    pPot2Raw = rawP2;
  }
...
void toneConnected() {
  // "Power up lock-on" — R2D2-ish rising sweep + confirmation pip
  tone(PIN_BUZZ, 600,  40);  delay(45);
  tone(PIN_BUZZ, 900,  40);  delay(45);
  tone(PIN_BUZZ, 1200, 40);  delay(45);
  tone(PIN_BUZZ, 1600, 40);  delay(45);
  tone(PIN_BUZZ, 2000, 40);  delay(45);
  noTone(PIN_BUZZ);           delay(30);   // brief silence = punch
  tone(PIN_BUZZ, 2400, 120); delay(130);  // final confirmation pip
}

void toneDisconnected() {
  // "Signal lost" — descending wail + two sharp warning pulses
  tone(PIN_BUZZ, 2000, 50);  delay(55);
  tone(PIN_BUZZ, 1600, 50);  delay(55);
  tone(PIN_BUZZ, 1200, 50);  delay(55);
  tone(PIN_BUZZ, 800,  50);  delay(55);
  tone(PIN_BUZZ, 500,  150); delay(160);  // low wail
  noTone(PIN_BUZZ);           delay(60);
  tone(PIN_BUZZ, 900,  60);  delay(70);   // double warning pulse
  noTone(PIN_BUZZ);           delay(40);
  tone(PIN_BUZZ, 900,  60);  delay(70);
}
// ================================================================
//  ESP-NOW CALLBACKS
// ================================================================
...
void startup() {
  ...
  while (true) {
    ...
    if (sw1OK && sw2OK && p1OK && p2OK && batOK) {
      ...
      tone(PIN_BUZZ, 1000, 80);
      delay(130);
      tone(PIN_BUZZ, 1400, 80);
      delay(900);  // let Nano linger on the normal screen a moment
      return;
    }
